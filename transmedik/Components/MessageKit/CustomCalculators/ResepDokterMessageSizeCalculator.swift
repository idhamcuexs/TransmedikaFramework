//
//  ResepDokterMessageSizeCalculator.swift
//  Pasien
//
...
open class ResepDokterMessageSizeCalculator: MessageSizeCalculator {
        ...
        if case .attributedText(let attributedItem) = message.kind {
            ...
            if json["recipes"].exists() {
                ...
                for (key,subJson):(String, JSON) in json["recipes"] {
                    text.append(NSAttributedString(string: "\(subJson["medicines_name"].string ?? "")  \(subJson["qty"].string ?? "")\r\n" , attributes: [.font: UIFont.systemFont(ofSize: 14)]) )
                }
